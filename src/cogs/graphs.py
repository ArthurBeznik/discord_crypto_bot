...
class Graphs(commands.Cog, name="graph"):
    ...
    async def generate_graph(
        self, interaction: discord.Interaction, crypto_id: str, period: str
    ) -> None:
        ...
        if df is None:
            embed = error_embed(
                "Error Fetching Data",
                f"Could not fetch data for {crypto_id}. Please try again.",
            )
        ...
        embed = success_embed(
            f"{crypto_id.upper()} Price Graph",
            f"Here's the price graph for {crypto_id.upper()} over the last {period}.",
        )
    ...
    async def provide_history(
        ...
        table = df.tail(
            12 if period == "1y" else 10
        ).to_string()  # Show 12 months if 1y, else 10 latest entries

        # Send the data as an embed
        embed = success_embed(
            f"Historical Data for {crypto_id.upper()}",
            f"Here is the historical data for {crypto_id.upper()} over the last {period}.\n\n```{table}```",
        )
        await interaction.response.send_message(embed=embed)
        logger.info(f"Provided historical data for {crypto_id} over the last {period}.")

    @app_commands.command(
        name="graph", description="Perform an action on cryptocurrency data."
    )
    @app_commands.rename(type="type", crypto="crypto", period="period")
    @app_commands.describe(
        type="graphic or history",
    ...
    async def graph(
        ...
        if type == "graphic":
            ...
            embed = error_embed(
                "Invalid Action",
                "Please choose a valid action: 'graphic' or 'history'.",
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
