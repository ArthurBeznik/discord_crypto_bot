...
class News(commands.Cog, name="news"):
    ...
    @app_commands.command(
        name="news",
        description="Display the latest global or specific cryptocurrency news.",
    )
    @app_commands.describe(
        type="Displays global news on cryptocurrency or for one specific crypto",
        crypto="Name or symbol of the cryptocurrency. Required if action is 'crypto'.",
    )
    @app_commands.autocomplete(crypto=crypto_autocomplete)
    async def news(
        self,
        interaction: discord.Interaction,
        type: Literal["global", "crypto"],
        crypto: str = None,
    ) -> None:
        """
        ...
        if type == "global":
            ...
            if crypto:
                ...
                await interaction.response.send_message(
                    "Please provide a cryptocurrency name or symbol for the 'crypto' action.",
                    ephemeral=True,
                )
                return
        ...
        if news_list:
            ...
            if type == "crypto" and crypto:
                await interaction.response.send_message(
                    f"No news found for {crypto.upper()}. Please try again with a different cryptocurrency."
                )
            else:
                await interaction.response.send_message(
                    "No global news found at the moment. Please try again later."
                )
