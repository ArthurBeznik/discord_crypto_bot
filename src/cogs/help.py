...
class Help(commands.Cog, name="help"):
    ...
    async def get_command_autocomplete(
        ...
        commands = [
            ...
            if cmd.name.startswith(current.lower())
        ]
        return [app_commands.Choice(name=cmd, value=cmd) for cmd in commands]

    @app_commands.command(
        name="help", description="Displays information about commands"
    )
    @app_commands.describe(
        type="Displays help for all commands or for one specific command",
    ...
    async def help(
        ...
        if type == "all":
            ...
            embed.add_field(
                name="For more details on a specific command",
                value="/help command",
                inline=False,
            )
            ...
            if command:
                ...
                if isinstance(command, app_commands.Group):
                    ...
                    embed = discord.Embed(
                        title=f"Help: Command Group - {command.name}", color=0x00A300
                    )
                    for subcommand in command.commands:
                        embed.add_field(
                            name=f"/{subcommand.qualified_name}",
                    ...
                    embed = discord.Embed(
                        title=f"Help: Command - {command.name}", color=0x00A300
                    )
                    embed.add_field(
                        name="Description",
                    ...
                    embed.add_field(
                        name="Usage", value=f"/{command.name}", inline=False
                    )
                    await interaction.response.send_message(embed=embed)
            else:
                await interaction.response.send_message(
                    f"No command found with the name `{name}`", ephemeral=True
                )
        else:
            await interaction.response.send_message(
                "Invalid usage. Please specify a valid command.", ephemeral=True
            )
