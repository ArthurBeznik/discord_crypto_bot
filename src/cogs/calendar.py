...
class CryptoCalendar(commands.Cog):
    ...
    def fetch_crypto_news(self):
        ...
        """
        url = ""
        headers = {"Accepts": "application/json", "API_KEY": CALENDAR_API_KEY}
        params = {
    ...
    @app_commands.command(
        name="calendar",
        description="Displays major economic events that could affect the crypto market.",
    )
    async def calendar(self, interaction: discord.Interaction) -> None:
        ...
        if crypto_news:
            embed = discord.Embed(
                title="Crypto Market News", color=discord.Color.blue()
            )
            for article in crypto_news.get("data", []):
                ...
                if title and url:
                    embed.add_field(
                        name=title, value=f"[Read more]({url})", inline=False
                    )
            await interaction.response.send_message(embed=embed)
        else:
            await interaction.response.send_message(
                "Could not fetch crypto news at this time."
            )
