...
class Info(commands.Cog):
    ...
    @app_commands.command(
        name="info",
        description="Display comprehensive market information for a cryptocurrency.",
    )
    ...
    async def info(self, interaction: discord.Interaction, crypto: str) -> None:
        ...
        if data is None:
            embed = error_embed(
                "Error Fetching Data",
                f"Could not fetch data for {crypto}. Please try again.",
            )
        ...
        volume_24h = (
            data.get("market_data", {}).get("total_volume", {}).get("usd", "N/A")
        )
        ...
        embed.add_field(
            name="Market Cap",
            value=f"${market_cap:,.2f}" if market_cap != "N/A" else "N/A",
            inline=False,
        )
        embed.add_field(
            name="24h Trading Volume",
            value=f"${volume_24h:,.2f}" if volume_24h != "N/A" else "N/A",
            inline=False,
        )
        embed.add_field(
            name="Volume/Market Cap Ratio (24h)",
            value=volume_market_cap_ratio,
            inline=False,
        )
        embed.add_field(
            name="Circulating Supply",
            value=f"{circulating_supply:,}" if circulating_supply != "N/A" else "N/A",
            inline=False,
        )
        embed.add_field(
            name="Total Supply",
            value=f"{total_supply:,}" if total_supply != "N/A" else "N/A",
            inline=False,
        )
        embed.add_field(
            name="Max Supply",
            value=f"{max_supply:,}" if max_supply != "N/A" else "N/A",
            inline=False,
        )
        embed.add_field(
            name="Fully Diluted Market Cap",
            value=f"${fully_diluted_market_cap:,.2f}"
            if fully_diluted_market_cap != "N/A"
            else "N/A",
            inline=False,
        )
