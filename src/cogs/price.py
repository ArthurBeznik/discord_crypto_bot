...
class Price(commands.GroupCog, name="price"):
    ...
    @app_commands.command(
        name="single", description="Get the price of a single cryptocurrency"
    )
    ...
    async def price_single(self, interaction: discord.Interaction, crypto: str) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Price fetched successfully for {crypto}: ${data.get(crypto_id, {}).get('usd', 'N/A')}"
                )
                embed = success_embed(price_message, "")
                await interaction.response.send_message(embed=embed)
            else:
                logger.error(
                    f"Error fetching prices for {crypto}. Status code: {response.status_code}"
                )
                await interaction.response.send_message(
                    "Error fetching the prices. Please try again."
                )
    ...
    @app_commands.command(
        name="multiple", description="Get the price of multiple cryptocurrencies"
    )
    @app_commands.rename(cryptos="cryptos")
    @app_commands.describe(
        cryptos="Names or symbols of the cryptocurrencies, separated by spaces"
    )
    async def price_multiple(
        self, interaction: discord.Interaction, cryptos: str
    ) -> None:
        """_summary_
        ...
        try:
            ...
            for crypto in crypto_list_input:
                crypto_id = self.bot.crypto_map.get(crypto.lower())
                if crypto_id:
                    resolved_cryptos.append(
                        (crypto, crypto_id)
                    )  # Store both the input and resolved id
                else:
            ...
            if not resolved_cryptos and unrecognized_cryptos:
                await interaction.response.send_message(
                    f"Unrecognized cryptocurrencies: {', '.join(unrecognized_cryptos)}"
                )
                logger.warning(
                    f"All provided cryptocurrencies were unrecognized: {', '.join(unrecognized_cryptos)}"
                )
                return
            ...
            if response.status_code == 200:
                ...
                for crypto, crypto_id in resolved_cryptos:
                    ...
                    response_message.append(
                        f"The current price of {crypto} is ${price}"
                    )
                ...
                logger.error(
                    f"Error fetching prices for multiple cryptos. Status code: {response.status_code}"
                )

        except Exception as e:
            logger.error(f"Exception in price_multiple: {str(e)}")
            await interaction.response.send_message(
                "An error occurred while fetching the prices. Please try again later."
            )
