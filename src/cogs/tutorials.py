...
from utils.config import TUTORIALS_FILE
...
class Tutorial(commands.Cog, name="tutorial"):
    ...
    async def topic_autocomplete(
        ...
        return [
            app_commands.Choice(name=topic.capitalize(), value=topic)
            for topic in topics
        ]

    @app_commands.command(
        name="tutorial",
        description="Generate an interactive tutorial based on a specific topic.",
    )
    @app_commands.describe(
        topic="The topic for which you want a tutorial (e.g., wallet, technical analysis)"
    )
