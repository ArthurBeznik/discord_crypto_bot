...
class ListCrypto(commands.Cog, name="listCrypto"):
    ...
    @app_commands.command(
        name="list_crypto",
        description="Command to list available cryptocurrencies from the CoinGecko API.",
    )
    async def list_crypto(self, interaction: discord.Interaction) -> None:
        ...
        """
        logger.info(
            f"Listing available cryptos, user: {interaction.user} (ID: {interaction.user.id})"
        )
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Error fetching cryptocurrencies. Status code: {response.status_code}"
            )
            await interaction.response.send_message(
                "Error fetching the list of cryptocurrencies. Please try again later."
            )
